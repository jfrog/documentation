...
* **Code structure.** Make sure the plugin code is structured similarly to the [jfrog-cli-plugin-template](https://github.com/jfrog/jfrog-cli-plugin-template). Specifically, it should include a _commands_ package, and a separate file for each command.
...
* Set the ID of the configured server as the value of the _JFROG\_CLI\_PLUGINS\_SERVER_ environment variable.
* If you wish the name of the plugins repository to be different from _jfrog-cli-plugins_, set this name as the value of the _JFROG\_CLI\_PLUGINS\_REPO_ environment variable.
...
If your plugin also uses [external resources](developer-guide.md#having-your-plugin-use-external-resources), you should place the resources under the following path -
